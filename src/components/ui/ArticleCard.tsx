import { Bookmark, BookmarkCheck, Clock, Eye, Heart } from "lucide-react";
...
export const ArticleCard: React.FC<ArticleCardProps> = ({
      ...
      <article className={`group ${className}`}>
        <Link to={`/article/${post.slug}`} className="flex items-start gap-3">
          <span className="text-3xl font-black text-gray-200 dark:text-gray-700 font-['Manrope'] leading-none mt-0.5 w-7 flex-shrink-0">
            {String(parseInt(className?.match(/\d+/)?.[0] || "1")).padStart(
              2,
              "0",
            )}
          </span>
      ...
      >
        <Link to={`/article/${post.slug}`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <h3 className="text-base font-bold text-white line-clamp-2 font-['Manrope'] leading-tight">
              {post.title}
            </h3>
  ...
  return (
    <article
      className={`bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 group ${className}`}
    >
      <Link to={`/article/${post.slug}`}>
        <div className="relative overflow-hidden h-48">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            {post.isBreaking && (
              <span className="bg-[#DC2626] text-white text-xs font-bold px-2 py-0.5 rounded animate-pulse">
                BREAKING
              </span>
            )}
            <span
          ...
          {post.type === "story" && (
            <div className="absolute top-3 right-3 bg-purple-600 text-white text-xs font-bold px-2 py-0.5 rounded">
              STORY
            </div>
          )}
          {post.type === "opinion" && (
            <div className="absolute top-3 right-3 bg-amber-500 text-white text-xs font-bold px-2 py-0.5 rounded">
              OPINION
            </div>
          )}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          <Link
            ...
            className="text-xs text-gray-500 dark:text-gray-400 hover:text-[#1E3A8A] dark:hover:text-blue-400 transition-colors font-medium"
          >
            {post.author}
          </Link>
